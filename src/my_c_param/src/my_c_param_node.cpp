...
my_c_publisher_node::my_c_publisher_node() : Node("vel_publisher")
{
  // 声明一个速度的参数，初始速度为1
  this->declare_parameter<float>("speed", 1.0);
  ...
  param_timer_ = this->create_wall_timer(
      1000ms, std::bind(&my_c_publisher_node::param_cb, this));
}
...
void my_c_publisher_node::param_cb()
{
  this->get_parameter("speed", vel_);
  RCLCPP_INFO(this->get_logger(), "the velocity is %f", vel_);
}

int main(int argc, char **argv)
{
  rclcpp::init(argc, argv);
  // 使用rclcpp::spin创建my_c_publisher_node，进入自旋锁
  rclcpp::spin(std::make_shared<my_c_publisher_node>());
  rclcpp::shutdown();
  return 0;
}